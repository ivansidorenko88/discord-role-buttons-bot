  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (!interaction.memberPermissions?.has(PermissionFlagsBits.ManageRoles)) {
        return interaction.reply({ content: 'Нужно право **Управление ролями**.', ephemeral: true });
      }
      ...
      if (sub === 'add') {
        ...
        if (role.id === interaction.guild.id) {
          return interaction.reply({ content: 'Нельзя использовать роль @everyone.', ephemeral: true });
        }

        if (role.managed) {
          return interaction.reply({ content: 'Эта роль управляется интеграцией и не может выдаваться ботом.', ephemeral: true });
        }

        const me = interaction.guild.members.me;
        if (!me) {
          return interaction.reply({ content: 'Не удалось определить роль бота.', ephemeral: true });
        }
        ...
        if (existingIndex >= 0) config.buttons[existingIndex] = item;
        else config.buttons.push(item);
      ...
      if (sub === 'remove') {
        ...
        if (config.buttons.length === before) {
          return interaction.reply({ content: 'Кнопка для этой роли не найдена.', ephemeral: true });
        }
      ...
      if (sub === 'list') {
        if (!config.buttons.length) {
          return interaction.reply({ content: 'Кнопки пока не настроены.', ephemeral: true });
        }
    ...
    if (interaction.isButton() && interaction.customId.startsWith('role:')) {
      ...
      // Критически важно: кнопки работают только в фиксированном канале.
